...
#include <QVBoxLayout>
#include <QHBoxLayout>
#include <corecrt_math_defines.h> // Для M_PI в Windows
...
void graph_scene::generate_random_graph() {
    int r = rand() % 4 + 6;
    QVector<QVector<int>> matrix(r, QVector<int>(r, 0));
    for (int i = 1; i < r; i++) {
        for (int j = 0; j < r; j++) {
            int r1 = rand() % 10;
...
void graph_scene::generate_bt(int count) {
    QVector<QVector<int>> matrix(count, QVector<int>(count, 0));
...
main_window::main_window(QWidget* parent)
    ...
    sample_matrix << QVector<int>{0, 10, 0, 5, 0, 0}
                  << QVector<int>{10, 0, 8, 0, 0, 0}
                  << QVector<int>{0, 8, 0, 3, 2, 0}
                  << QVector<int>{5, 0, 3, 0, 6, 0}
                  << QVector<int>{0, 0, 2, 6, 0, 4}
                  << QVector<int>{0, 0, 0, 0, 4, 0};
